...
/*
Note: This function will not be part of the feature implementation
it only exists for us to inspect the SQL query generated by the getGrants function below.
*/
async function getGrantsToString({
    currentPage, perPage, tenantId, filters, orderBy, searchTerm, orderDesc,
} = {}) {
    console.log(tenantId);
    const sql = await knex(TABLES.grants)
...
await db.getGrants({
    ...
    filters: {
        agencyCriteria: {
            eligibilityCodes: [ '00', '01', '02', '04', '05', '06', '07', '08', '11', '12', '13', '20', '21', '22', '23', '25', '99' ],
            includeKeywords: [ 'fire' ],
            excludeKeywords: [ 'emergency' ]
        },
        interestedByAgency: null,
        interestedByUser: null,
        assignedToAgency: null,
...
async function getGrants({
    currentPage, perPage, tenantId, filters, orderBy, searchTerm, orderDesc, tosqlString
} = {}) {
    console.log(currentPage, ' ', perPage, ' ', tenantId, ' ', filters, ' ', orderBy, ' ', searchTerm, ' ', orderDesc);
    if (tosqlString) {
        getGrantsToString({
            currentPage, perPage, tenantId, filters, orderBy, searchTerm, orderDesc,
        });
